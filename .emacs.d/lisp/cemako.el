...
(defcustom cemako-dir-name-function 'cemako-default-build-dir-function
...
(defun cemako--validate (project-data &optional tag)
  "Return t if current project can be built or run.
...
TAG indicates the action being validated, e.g. `compile' or `run'."
  (not
   (cl-case (cemako--validity project-data)
     (:build-dir-missing
      (message "cemako %s: No build dir (%s)\nDo you need to initialize CMake?"
               (or tag "compile")
               (cemako--get-build-dir project-data)))
     (:data-missing
      (message "cemako %s: Not a valid project ; no .cemako.el data found (project root is %s)"
	           (or tag "compile")
...
(defun cemako--get-build-dir (project-data)
  ...
  (concat (cemako--get-project-root) (funcall cemako-dir-name-function project-data)))
...
(defun cemako--invoke-build-current (&optional sentinel)
...
If SENTINEL is non-nil, use it as the process sentinel."
  (let* ((project-data (cemako--read-project-data))
    ...
    (when (cemako--validate project-data)
      (let* ((project-data (cemako--read-project-data)) ;; FIXME redundant reads
...
(defun cemako--completing-read-profile-name (project-data must-match)
  ...
  (let* ((profiles (cemako--get-profiles project-data))
    ...
    (completing-read "Select CMake profile: " choices nil must-match)))
...
(defun cemako-edit-profile (&optional profile-name profile-options)
  ...
  (cemako--with-project-data
   (let* ((profiles (or (cdr (assoc 'profiles project-data)) '()))
          ...
          (profile-name (or profile-name (cemako--completing-read-profile-name project-data nil)))
          (profile-options (or profile-options (read-string "CMake profile options: " "-DCMAKE_BUILD_TYPE=Debug"))))
...
(defun cemako-run-cmake ()
  ...
  (interactive)
  (let* ((project-data (cemako--read-project-data))) ;; FIXME
...
(defun cemako--get-available-targets (project-data target-types target-executables)
...
(defun cemako-select-target (&optional target-name)
  ...
  (cemako--with-project-data
   (let* ((target-types (make-hash-table :test 'equal))
          ...
          (target-names (cemako--get-available-targets project-data target-types target-executables))
